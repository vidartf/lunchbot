...
SLACK_TOKEN = config.get(
    'Slack', 'token', fallback=os.environ.get("SLACK_API_TOKEN", None)
)
FACEBOOK_SECRET = config.get(
    'Facebook', 'secret', fallback=os.environ.get("FACEBOOK_API_SECRET", None)
)
FACEBOOK_ID = config.get(
    'Facebook', 'id', fallback=os.environ.get("FACEBOOK_API_ID", None)
)

SLACK_CHANNELS = [
    c.strip()
    for c in config.get('Slack', 'channels', fallback='lunchbotdev').split(',')
]
